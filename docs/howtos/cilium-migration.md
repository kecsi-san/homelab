...
|--------|----------|--------|
| Traefik IngressRoute CRDs (`traefik.io/v1alpha1`) | n/a | Traefik-owned CRDs, unaffected by k8s API changes |
| ArgoCD, cert-manager, Longhorn, Sealed Secrets CRDs | n/a | Operator-owned CRDs, unaffected |
| `apps/v1` Deployments, `v1` Services/ConfigMaps | n/a | Stable APIs, no changes |
