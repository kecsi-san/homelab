...
- **Pin volatile Homebrew formulas**: an unpinned formula (`state: present`, no version) can silently major-version-bump on `upgrade-local.yml`'s `brew upgrade`. Helm did exactly this (3→4) and broke ArgoCD's Server-Side-Apply assumptions, pinned to `helm@3` in `setup_kube-extra` from 2026-07-19 until the actual break (`argocd-cm`/`argocd-rbac-cm` co-managed by the Helm chart and `argocd-config`'s raw manifests) was fixed for real and re-verified against the live v4 binary on both clusters (2026-09-06, see `TODO.md`'s "Helm 4 migration" item); unpinned since. Apply the same pin-first pattern to any other tool where a breaking major bump would hurt, until the actual incompatibility is understood and fixed rather than just avoided.
