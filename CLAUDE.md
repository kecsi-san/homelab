...
- **`.hu` domain names never appear literal in Markdown**: the two personal `.hu` domains (registered at Integrity.hu, unrelated to the intentionally-public `kecskemethy.org` cluster domain used throughout this repo) get shortened to their first letter + `.hu` wherever they'd otherwise appear in a `.md` file — `k*.hu`, `l*.hu` — subdomains keep their prefix (`vault.k*.hu`, `mail.l*.hu`). Applies repo-wide: `docs/**/*.md`, `README.md`, role READMEs, `TODO.md`. `docs/howtos/ec2-rebuild-plan.md` and `ec2-ebs-volumes.md` already anonymize every hosted domain (`.com`/`.net`/`.hu` alike) via a `<d1>`/`<d2>` placeholder scheme; leave that convention as-is rather than converting it to `k*`/`l*`. Non-Markdown files (Ansible vars, Terraform, YAML) are unaffected — those already route real domain values through `secrets.yml`/Vault, not literal strings in tracked files.
