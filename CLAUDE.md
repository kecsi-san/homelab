...
| ~~kube-prometheus-stack~~ | monitoring | *(removed 2026-08-08)* | Was Prometheus + Alertmanager, scraping cluster + 4-node Debian `node_exporter`, ntfy webhook bridge; removed to free Longhorn disk space on `hped800g5` during a Postgres outage (its 43GiB TSDB PVC was the dominant consumer) — see TODO.md for reinstall follow-up. Grafana/kromgo have no datasource until it's back. |
