...
- [ ] **EC2 rebuild — Phase 6** — new instance provisioning + data migration + EIP cutover + old-instance decommission; Phases 1–5 (all Ansible roles: users, email, apache2, vault, unbound) are done; Terraform for the new instance + 4-volume EBS layout written 2026-07-11 (`module.ec2_edge`/`module.eip_edge` in `terraform/aws/main.tf`), plan verified clean, `terraform apply` not yet run; bootstrap design finalized 2026-07-12 (cloud-init handles admin→`kecsi` rename + EBS mount at first boot, dedicated `linuxbox2026` SSH keypair escrowed in Vault via two split KV entries) but not yet implemented in Terraform; see `docs/howtos/ec2-rebuild-plan.md` and `docs/howtos/ec2-ebs-volumes.md` for full status
