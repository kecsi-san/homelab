...
- [ ] **PR #37 `chore(helm): Update helm charts`** — clean/mergeable (6 Helm chart bumps across k3s+k8s: authentik, cert-manager, cnpg, reloader, sealed-secrets, traefik, kube-prometheus-stack; all patch/minor, no majors); k8s.yml runs are done, but holding until `post-k3s.yml` (currently mid-retry, touches ArgoCD itself on k3s) finishes cleanly, so we're not stacking a live ArgoCD chart sync on top of it
